...
function moveCrates(crates: string[][], [nm, from, to]: [number, number, number], reverse: boolean = false) {
  const nToMove = crates[from].length - nm;
  const toMove = reverse ? crates[from].slice(nToMove).reverse() : crates[from].slice(nToMove);
